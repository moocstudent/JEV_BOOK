/* =========================================================
   Curriculum data — 8 modules / 27 chapters
   ---------------------------------------------------------
...
const CHAPTERS = [
  ...
  {
    id: "t25", code: "SO4", moduleId: "m1", difficulty: 2, hours: 5, prereq: ["t3"], viz: "archLab",
    props: ["System 1 vs System 2", "解码器 vs 编码器", "自回归 vs 一次前向", "训练目标", "服务栈", "为什么更快"],
    title: { zh: "ChatGPT 与 Jev:两种模型的解剖对比", en: "ChatGPT vs Jev: Two Models, Dissected" },
    summary: {
      zh: "这一章把前三章的线索收成一张对照表,回答一个你一定会被问到的问题:Jev 这类模型和 ChatGPT 到底差在哪,为什么快那么多。差别不是「同一种东西的大小版本」,而是两种在解决不同问题的模型。ChatGPT 建模的是「给定上文,下一个 token 是什么」,产出的是语言;Jev 建模的是「给定状态和几个类型化问题,每个问题的答案分布是什么」,产出的是带校准概率的决策。TypeSafe 把这个对立叫做 System 2(慢、审慎、生成式推理)对 System 1(快、反射式判断)。这一章沿着四条轴把两者拆开:算法(自回归逐 token 解码 vs 一次前向读分类头)、架构(解码器-only 的因果注意力、上千亿参数 vs 编码器的双向注意力、几亿参数)、训练目标(下一个 token 的似然加 RLHF vs 直接优化校准的严格适当评分规则)、以及服务栈(vLLM/TensorRT-LLM 加 KV cache 管理 vs 一个 FastAPI 包一层、显存常数)。然后把「为什么快」讲成三个相乘的效应:一次前向对 N 次前向、模型小一百到一千倍、以及没有采样循环和随序列增长的 KV cache。诚实的边界也在这一章:Jev 的内部从未公开到可复现,这张表里的架构是开源平替(Laya、Von)独立再推导出来的,不是从 Jev 拆出来的;而且这不是一个更好的 ChatGPT,它只能回答你预先定义好答案空间的问题,是一个用在 LLM 旁边或前面的快速决策头,不是替代品。",
      en: "This chapter gathers the threads of the first three into one comparison and answers the question you will inevitably be asked: how does a model like Jev differ from ChatGPT, and why is it so much faster? The difference is not a size variant of one thing but two models solving different problems. ChatGPT models 'given the context, what is the next token' and produces language; Jev models 'given a state and some typed questions, what is the distribution of each answer' and produces calibrated decisions. TypeSafe frames this as System 2 (slow, deliberate, generative reasoning) versus System 1 (fast, reflexive judgement). The chapter takes the two apart along four axes: algorithm (autoregressive token-by-token decoding versus a single forward pass reading classification heads), architecture (a decoder-only causal-attention model of hundreds of billions of parameters versus a bidirectional encoder of hundreds of millions), training objective (next-token likelihood plus RLHF versus a strictly proper scoring rule that optimises calibration directly), and serving stack (vLLM/TensorRT-LLM with KV-cache management versus a FastAPI wrapper with constant memory). Then it explains 'why faster' as three multiplicative effects: one forward pass against N, a model one hundred to one thousand times smaller, and no sampling loop or KV cache that grows with the sequence. The honest boundary is here too: Jev's internals were never published reproducibly, so the architecture in this table is what the open alternatives (Laya, Von) independently re-derived rather than what was taken from Jev; and this is not a better ChatGPT — it can only answer questions whose answer space you defined in advance, a fast decision head used beside or in front of an LLM, not a replacement.",
    },
    objectives: [
      { zh: "沿算法、架构、训练、服务四条轴说清两类模型的区别", en: "State the difference along algorithm, architecture, training and serving" },
      { zh: "把「为什么快」拆成三个相乘的效应,而不是一句「更小」", en: "Decompose 'why faster' into three multiplicative effects, not just 'smaller'" },
      { zh: "说清 Jev 不是替代 ChatGPT,而是一个不同用途的工具", en: "Explain that Jev does not replace ChatGPT but is a different tool" },
    ],
    outline: [
      { zh: "System 1 与 System 2:一个提法背后的两种问题", en: "System 1 and System 2: two problems behind one framing" },
      { zh: "四条轴:算法 / 架构 / 训练 / 服务", en: "Four axes: algorithm / architecture / training / serving" },
      { zh: "为什么快:三个相乘的效应", en: "Why faster: three multiplicative effects" },
      { zh: "诚实边界:再推导,不是拆解;工具,不是替代", en: "Honest boundary: re-derived not reverse-engineered; a tool not a replacement" },
    ],
  },
  ...
  {
    id: "t26", code: "LY4", moduleId: "m4", difficulty: 3, hours: 5, prereq: ["t10"], viz: "runtimeLab",
    props: ["三个位置", "pip 库 vs 权重", "DecisionModel", "编码器 + 标记打分头", "一次前向的形状流", "本地 CPU 推理"],
    title: { zh: "Laya 的运行时:从 load() 到 logits 的全过程", en: "Inside the Runtime: from load() to logits" },
    summary: {
      zh: "这一章把开源 Laya「到底在哪里、怎么跑」拆到源码级,回答一个所有人都会混淆的问题:模型在本地、在一个外部库里、还是在远端?答案是三个不同的东西住在三个不同的地方。第一,你写的脚本(比如本仓库的 laya_check.py)在本地,它**不含任何模型和权重**,只是编排加纯 Python 的统计(ECE、温度网格搜索、门槛表)。第二,`laya` 这个 pip 库也在本地,但它也**不含权重**——它只有九个很小的 .py 文件,是加载和运行模型的**代码**,真正的张量计算交给它依赖的 torch 和 transformers。第三,权重才是那个约四亿参数的神经网络,它从 Hugging Face(`convaiinnovations/laya`)**远端下载、本地缓存**到 `~/.cache/huggingface`。关键结论:一旦权重缓存好,推理**百分之百在你本地的 CPU 上跑**,调用 `agent.predict` 不会向任何远端 API 发一个字节——这正是它和 Jev(一个你 POST 过去的托管远端 API)的根本区别。然后这一章讲清那个外部库的模型原理:`DecisionModel` = 一个双向编码器(ModernBERT-large / mmBERT-base)+ 一个类型嵌入 + 一个两层的小 head + 一个 `scorer`(把任意一个 token 的隐向量映射成一个标量)。它最关键的设计是**没有固定的分类头**——选项不是分类头上的神经元,而是被拼进输入的文本,每个选项在序列里留一个「标记」位置;一次前向之后,模型在这些标记位置上 `gather` 出隐向量,用同一个 scorer 打成每个选项一个 logit,再除以分桶温度、softmax。这个「给每个选项标记打分再 softmax」的机制,一句话解释了它为什么快(一次前向、无生成循环、无 KV cache)、也解释了它为什么标签一多就垮(77 个标记都在同一条序列里抢 head_max_len 的位置)。最后点出那条诚实的接缝:这个库能一字不差地复现 Laya 的**推理**,但让概率变得校准的**训练**(RLCD)只烘焙在权重里、只被近似——这就是为什么每个「开源 Jev」都是再推导,不是复刻。",
      en: "This chapter takes 'where the open Laya actually is and how it runs' down to source level, answering the question everyone conflates: is the model local, in an external library, or remote? The answer is three different things in three different places. First, the script you write (this repo's laya_check.py, say) is local and contains no model and no weights — it is orchestration plus pure-Python statistics (ECE, a temperature grid search, the threshold table). Second, the `laya` pip package is also local but also holds no weights — nine small .py files that are the code to load and run the model, with the real tensor compute handed to the torch and transformers it depends on. Third, the weights are the ~400M-parameter network itself, downloaded from Hugging Face (`convaiinnovations/laya`) and cached locally to `~/.cache/huggingface`. The key conclusion: once the weights are cached, inference runs entirely on your local CPU, and calling `agent.predict` sends not a byte to any remote API — precisely the difference from Jev, a hosted remote API you POST to. The chapter then explains the library's model: `DecisionModel` = a bidirectional encoder (ModernBERT-large / mmBERT-base) + a type embedding + a small two-layer head + a `scorer` that maps any token's hidden vector to a single scalar. Its defining choice is that there is no fixed classification head — options are not neurons on a head but text spliced into the input, each leaving a 'marker' position in the sequence; after one forward pass the model gathers the hidden vectors at those markers and scores each with the shared scorer into one logit per option, then divides by a bucketed temperature and softmaxes. This 'score each option marker and softmax' mechanism explains in one line why it is fast (one pass, no generation loop, no KV cache) and why it collapses on many labels (77 markers all competing for head_max_len room in one sequence). It closes on the honest seam: the library reproduces Laya's inference exactly, but the training (RLCD) that made the probabilities calibrated is baked into the weights and only approximated — which is why every 'open Jev' is a re-derivation, not a clone.",
    },
    objectives: [
      { zh: "分清三个位置:本地脚本 / pip 库 / 远端缓存权重,并知道推理在哪跑", en: "Separate the three locations — local script / pip library / remote-cached weights — and know where inference runs" },
      { zh: "说清 DecisionModel 的结构:编码器 + 类型嵌入 + head + 标记 scorer", en: "State DecisionModel's structure: encoder + type embedding + head + marker scorer" },
      { zh: "用「给每个选项标记打分再 softmax」解释快与高基数崩塌", en: "Use 'score each marker then softmax' to explain both the speed and the high-cardinality collapse" },
    ],
    outline: [
      { zh: "三个位置:脚本 / 库 / 权重", en: "Three locations: script / library / weights" },
      { zh: "加载:snapshot_download + safetensors + build_model", en: "Loading: snapshot_download + safetensors + build_model" },
      { zh: "一次前向的六步:从 input_ids 到每选项一个 logit", en: "Six steps of the forward pass: from input_ids to one logit per option" },
      { zh: "诚实接缝:能复现推理,不能复现训练", en: "The honest seam: inference reproduced, training only approximated" },
    ],
  },
  ...
  {
    id: "t27", code: "OP4", moduleId: "m7", difficulty: 3, hours: 5, prereq: ["t21", "t12"], viz: "reviewLab",
    props: ["分诊而非评审", "choice k=3 判决", "score 严重度", "按 hunk 过闸", "cⁿ 推送崩塌", "MCP 工具", "本地运行 + 微调"],
    title: { zh: "代码评审当作类型化决策:给 TaskaaS 接一个 MCP 推送闸门", en: "Code Review as Typed Decisions: an MCP Push Gate for TaskaaS" },
    summary: {
      zh: "这一章回答三个连在一起的问题。第一,System One 模型能不能做代码评审?能做分诊,不能写评审:它不生成任何东西,所以没法解释 bug、指出行号或给补丁,但它能在一次前向里回答「这一块能不能跳过人工、是哪类风险、错了有多严重」——而这正是吃掉大部分评审时间的那件事。第二,在 choice / score / noul 这套三元语言里它属于哪个类型?核心判决是一个 k=3 的 choice(approve / request_changes / needs_human),不是 noul——看似二元,其实有「判断不了」这第三种结局;严重度用 score;「碰没碰鉴权、有没有测试」这类事实直接用正则算,不问模型。第三,怎么接进 TaskaaS 做推送评审?写一个 MCP 服务器 typed-review,暴露一个工具 review_push:把 diff 切成 hunk(英文 checkpoint 给文档留的大约 300 token 决定了单位),每块发一次 /v1/systemone(Jev 或内网的 laya-serve,同一协议只改 URL),不对称过闸——只有高置信度的 approve 自动放行,高置信度的 request_changes 交给 LLM 写评论,其余交给人。决策台会让你看到一个反直觉的结果:推送整体自动放行的概率是 cⁿ,按推送算的自动化会崩塌,真正的收益是评审者只看没放行的那些块。这一章也把「用 Laya 的功能增强 Jev」落成具体的一张表:本地运行(源代码不出网络)、同协议自托管、用合并历史这份免费标签做微调、按语言路由。诚实边界:本书没有实测代码评审,决策台是合成数据。",
      en: "This chapter answers three linked questions. First, can a System One model do code review? It can triage, not review: it generates nothing, so it cannot explain a bug, point at a line or propose a patch, but in one forward pass it can answer 'can this hunk skip a human, what kind of risk is it, how bad if wrong' — the part that eats most review time. Second, which type in the choice / score / noul ternary vocabulary? The core verdict is a k=3 choice (approve / request_changes / needs_human), not a noul — it looks binary but has a third outcome, 'can't tell'; severity is a score; facts like 'touches auth, has tests' are computed with a regex, not asked. Third, how does it plug into TaskaaS for push review? An MCP server, typed-review, exposes one tool, review_push: split the diff into hunks (the ~300 tokens the English checkpoint leaves for the document set the unit), send each hunk one /v1/systemone request (Jev or an in-network laya-serve — same protocol, only the URL changes), and gate asymmetrically — only a confident approve auto-passes, a confident request_changes goes to an LLM that writes the comment, everything else to a person. The bench shows a counter-intuitive result: a whole push auto-passes with probability cⁿ, so per-push automation collapses and the real win is that reviewers see only the hunks that did not pass. The chapter also turns 'improve Jev with Laya's functions' into a concrete table: local execution (code never leaves the network), same-protocol self-hosting, fine-tuning on your merge history as free labels, and language routing. Honest boundary: this book has not measured code review; the bench is synthetic.",
    },
    objectives: [
      { zh: "说清 System One 模型在代码评审里能做什么(分诊)、不能做什么(写评审)", en: "State what a System One model can do in code review (triage) and cannot (write the review)" },
      { zh: "把评审问题映射到 choice / score / noul,并说明为什么判决是 k=3 的 choice", en: "Map review questions onto choice / score / noul, and justify the verdict as a k=3 choice" },
      { zh: "写出一个 MCP 工具 review_push,让任何 MCP 宿主(包括 TaskaaS)按 hunk 过闸", en: "Write an MCP tool, review_push, that lets any MCP host (TaskaaS included) gate per hunk" },
      { zh: "用 cⁿ 解释为什么收益在评审工作量,而不在推送自动化率", en: "Use cⁿ to explain why the win is review load, not push automation" },
    ],
    outline: [
      { zh: "分诊,不是评审", en: "Triage, not the review" },
      { zh: "三元语言里的类型:choice 判决 + score 严重度,避开 noul", en: "The type in the ternary language: a choice verdict + a score severity, noul avoided" },
      { zh: "Laya 多出来的四样:本地、同协议、微调、路由", en: "What Laya adds: local, same protocol, fine-tuning, routing" },
      { zh: "token 预算决定单位:hunk 与 cⁿ", en: "The token budget sets the unit: the hunk, and cⁿ" },
      { zh: "通过 MCP 接进 TaskaaS:review_push、降级路径、推翻率", en: "Into TaskaaS over MCP: review_push, degradation path, override rate" },
    ],
  },
